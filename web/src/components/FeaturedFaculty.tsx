      ...
      <Tooltip.Root>
        <Tooltip.Trigger class="block w-full">
          ...
          <div class="text-center mt-6 space-y-2">
            ...
            <p class="text-muted font-serif italic">
              {props.member.shortName || props.member.name.split(' ')[0]}
            </p>
          </div>
        </Tooltip.Trigger>
        ...
        <Tooltip.Portal>
          <Tooltip.Content class="z-50 p-3 bg-surface/95 border border-border text-foreground rounded shadow-arcane max-w-xs">
            <div class="space-y-2">
              <p class="font-display text-primary">{props.member.name}</p>
              <p class="font-serif text-sm">{props.member.title}</p>
              {props.member.specialty && (
                <p class="text-xs italic text-muted/80">Specialty: {props.member.specialty}</p>
              )}
            </div>
            <Tooltip.Arrow />
          </Tooltip.Content>
