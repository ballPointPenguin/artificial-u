...
class ImageService:
    """
    Service for generating images using Gemini and GPT Image models.

    Generated images are stored in a configurable storage backend (MinIO/S3).
    """
    ...
    def _determine_backend(self, model_name: str) -> str:
        ...
        if model_name.startswith("gemini-"):
            return "gemini"
        elif model_name in {"gpt-image-2", "gpt-image-2.5-flare"}:
            return "openai"
        logger.error(f"Unsupported image generation model: {model_name}")
    ...
    async def _generate_gemini_image(
        ...
        *,
        ...
        logger.info(f"Using generate_content API for {model_name} (Gemini image model)")
    ...
    async def generate_course_image(
        ...
        # Final fallback to OpenAI's fast, high-quality GPT Image 2.5 Flare.
