...
def mock_api_service(monkeypatch):
    ...
    def _mock_create_lecture(lecture_data: LectureCreate, created_by=None):
        ...
        return Lecture(
            ...
            timeline_url=lecture_data.timeline_url,
            images_timeline_url=lecture_data.images_timeline_url,
            word_count=lecture_data.word_count,
    ...
    generated_lecture_mock = Lecture(
        ...
        audio_download_url=sample_lectures_base[0].audio_download_url,
        transcript_url=sample_lectures_base[0].transcript_url,
        timeline_url=sample_lectures_base[0].timeline_url,
        images_timeline_url=sample_lectures_base[0].images_timeline_url,
        word_count=sample_lectures_base[0].word_count,
...
@pytest.mark.unit
def test_list_lectures_with_topic_id_filter(client: TestClient, mock_api_service):
    """Test listing lectures with topic_id filter."""
    # Mock lectures for a specific topic
    topic_lectures = [lecture for lecture in sample_lectures_base if lecture.topic_id == 20]
    mock_api_service["list_lectures"].return_value = LectureListResponse(
        items=topic_lectures, total=len(topic_lectures), page=1, size=10, pages=1
    )

    response = client.get("/api/v1/lectures?topic_id=20")
    assert response.status_code == 200
    data = response.json()
    assert len(data["items"]) == len(topic_lectures)
