...
uv run artificial_u create-course -d "Computer Science" -t "AI" -c "CS101"
uv run artificial_u create-audio -c "CS101" -w 1 -n 1
...
- Python 3.14 only (`requires-python`, `.python-version`, CI, and CDK deployment)
...
```bash
uv run black artificial_u tests
uv run isort artificial_u tests
uv run flake8 artificial_u
```
...
When changing the database schema, **always prefer autogenerated migrations** from SQLAlchemy models. Do not hand-write schema migration files or guess `revision` / `down_revision` IDs — that produces awkward histories, broken chains, and merge conflicts. See `docs/DATABASE.md` for the full workflow and rollback guidance.

1. Update SQLAlchemy models in `artificial_u/models/`
2. Ensure the local database is at the current head (`upgrade head` if needed)
3. Autogenerate the migration
4. Review the generated file in `alembic/versions/` before applying

```bash
# Create new migration (always use --autogenerate for model-driven schema changes)
...
Use an empty `revision` (without `--autogenerate`) only for changes autogenerate cannot express, such as custom data backfills. Even then, let Alembic assign revision IDs — never invent them manually.
...
- **Alembic migrations**: Use `scripts/run_alembic.py` wrapper (not raw `alembic`), change models first, then `revision --autogenerate`; do not hand-author schema migrations or guess revision IDs (see `docs/DATABASE.md`)
...
- **uv environment**: Always use `uv run` or activate the virtualenv (`source .venv/bin/activate`) first
...
- `docs/DATABASE.md`: Migrations, rollbacks, and Alembic workflow
